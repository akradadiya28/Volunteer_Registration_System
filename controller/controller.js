let data = [];

const defaultController = (req, res) => {
    res.render('index.ejs');
}

const dataController = (req, res) => {
    const tableData = data;
    res.json(tableData); // Sending the data as JSON
}

const viewController = (req, res) => {
    res.render('view.ejs', { data });
}

const addController = (req, res) => {
    // console.log(req.body);
    let id = Math.floor(Math.random() * 1000);
    let regData = {
        id: id,
        fname: req.body.fname,
        lname: req.body.lname,
        email: req.body.email,
        num: req.body.num,
        skill: req.body.skill,
        action: `<a href="/editController/${id}"
                                    class="flex text-green-500 border-2 border-green-500 p-2 rounded-lg">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                                        class="bi bi-pencil-square h-6 w-6 text-green-500" viewBox="0 0 16 16">
                                        <path
                                            d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z" />
                                        <path fill-rule="evenodd"
                                            d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5z" />
                                    </svg>
                                    <span>Edit</span>
                                </a>
                                                        ||
                                <a href="/removeController/${id}"
                                    class="flex text-red-500 border-2 border-red-500 p-2 rounded-lg">
                                    <svg class="h-6 w-6 text-red-500" viewBox="0 0 24 24" fill="none"
                                        stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                        stroke-linejoin="round">
                                        <circle cx="12" cy="12" r="10" />
                                        <line x1="15" y1="9" x2="9" y2="15" />
                                        <line x1="9" y1="9" x2="15" y2="15" />
                                    </svg>
                                    <span>Remove</span>
                                </a>`

    }
    // console.log(regData);
    data = [...data, regData];
    res.redirect('/view')

}

const editController = (req, res) => {
    let { id } = req.params;

    let singleRec = data.find((rec => rec.id == id));
    // console.log(singleRec);
    res.render('edit.ejs', { singleRec });
}

const updateController = (req, res) => {
    let { id } = req.params;

    let updateData = data.find((rec => rec.id == id));

    updateData.fname = req.body.fname;
    updateData.lname = req.body.lname;
    updateData.email = req.body.email;
    updateData.num = req.body.num;
    updateData.skill = req.body.skill;

    // res.render('view.ejs', { data });

    res.redirect('/view')
}

const removeController = (req, res) => {
    let { id } = req.params;
    let newData = data.filter((rec => rec.id != id));
    data = newData;
    res.redirect('/view')

}

module.exports = { defaultController, addController, editController, updateController, removeController, viewController, dataController };